...
public class BenchmarkRunner : ConsoleAppBase
{
    ...
    /// <returns></returns>
    public async Task MagicOnion(string hostAddress = "http://127.0.0.1:5000", string iterations = "1", string duration = "30s", int concurrency = 50, int connections = 50, string reportId = "")
    {
    ...
    /// <param name="reportId"></param>
    /// <returns></returns>
    public async Task MagicOnionHub(string hostAddress = "http://127.0.0.1:5000", string iterations = "1", string duration = "30s", int concurrency = 50, int connections = 50, string reportId = "")
    {
        var iter = iterations.Split(',').Select(x => int.Parse(x.Trim())).ToArray();
        var benchmarker = new Benchmarker(_path, Context.Logger, Context.CancellationToken)
        {
    ...
    /// <param name="reportId"></param>
    /// <returns></returns>
    public async Task MagicOnionLongRunHub(int waitMilliseconds, string hostAddress = "http://127.0.0.1:5000", string iterations = "1", string duration = "30s", int concurrency = 50, int connections = 50, string reportId = "")
    {
        var iter = iterations.Split(',').Select(x => int.Parse(x.Trim())).ToArray();
        var benchmarker = new Benchmarker(_path, Context.Logger, Context.CancellationToken)
        {
    ...
    /// <param name="reportId"></param>
    /// <returns></returns>
    public async Task Grpc(string hostAddress = "http://127.0.0.1:5000", string iterations = "1", string duration = "30s", int concurrency = 50, int connections = 50, string reportId = "")
    {
        var iter = iterations.Split(',').Select(x => int.Parse(x.Trim())).ToArray();
        var benchmarker = new Benchmarker(_path, Context.Logger, Context.CancellationToken)
        {
    ...
    /// <param name="reportId"></param>
    /// <returns></returns>
    public async Task Api(string hostAddress = "http://127.0.0.1:5000", string iterations = "1", string duration = "30s", int concurrency = 50, int connections = 50, string reportId = "")
    {
        var iter = iterations.Split(',').Select(x => int.Parse(x.Trim())).ToArray();
        var benchmarker = new Benchmarker(_path, Context.Logger, Context.CancellationToken)
        {
