...
namespace Benchmark.ClientLib.Runtime
{
    ...
    public class UnaryResultPool<T> : IDisposable
    {
        public Task Completed => _completeTask.Task;

        private readonly ConcurrentQueue<(TaskCompletionSource tcs, UnaryResult<T> task)> _pool = new ConcurrentQueue<(TaskCompletionSource, UnaryResult<T>)>();
        ...
        public UnaryResultPool(int poolSize, CancellationToken ct)
        {
            _poolSize = poolSize;
            _autoResetEvent = new AutoResetEvent(false);
            _ct = ct;
            _timeoutTcs = new TaskCompletionSource();
            _completeTask = new TaskCompletionSource();
        ...
        /// <returns></returns>
        public Task RegisterAsync(Func<UnaryResult<T>> item)
        {
            lock (_lock)
            {
